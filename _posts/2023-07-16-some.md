...
$$p_{1} = \binom{k}{N}$$
![helpful picture?](https://github.com/n-diwan/n-diwan/blob/master/assets/images/lattice pieces.png?raw=true)
...
Whenever a lattice point piece is created, there are pieces formed in the intersection of these lattice point pieces. When \\(N-2\\) intersections of the \\(N-1\\) analog to a plane we get a line. For example, two planes intersect in a line. Our next set of pieces will be from lines created from the intersection of slices. Another way of thinking of thinking about these pieces are the pieces formed by the intersection of pieces eminating from the internal shape. These pieces intersect in lines so our goal is to find the number of lines. A basic counting argument reveals: 

$$p_{2} = \binom{k}{N-1}$$

The pattern is revealing itself, with our next set of pieces being the result of \\(N-3\\) intersection of the \\(N-1\\) dimensional slices. In 3D these are just the remaining pieces that aren't formed by lattice point or intersections of lattice point pieces. These are actually easy to spot, with them being the face pieces of our original interal piece. In 3D there are \\(k\\) such pieces, but in general there are: 
...
What I find cool about this formula is how as the dimension gets really high, the number of pieces approaches $$2^k$$. This makes intuitive sense because everytime we add a dimension there is a new way to cut every single piece in half, thus doubling the number of pieces. 
